	...
	if (pairs.size()) {
		...
		for (++it; it != pairs.end(); ++it) {
			...
			if (!last)
				...
				(*last).right = new typename bin_tree<Key, Prior>::node((*it).first, (*it).second,
...
void treap<Key, Prior>::push(const std::pair<Key, Prior> &pair) {
	...
	*last_b = new typename bin_tree<Key, Prior>::node(pair.first, pair.second, p.first, p.second, last);
}
...
typename bin_tree<Key, Prior>::node *
	...
	typename bin_tree<Key, Prior>::node *max = flag ? f : s, *min = flag ? s : f;
