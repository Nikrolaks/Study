...
private:
    ...
    friend std::ostream &operator <<(std::ostream &out, node &nod) {
        return nod.print(out);
    }
...
template <typename Key, typename Prior>
std::ostream &treap<Key, Prior>::node::print(std::ostream &out) {
    return out << "(" << kern.first << ", " << kern.second << ")";
}